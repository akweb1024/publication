...
const { ReviewRecommendation } = prismaEnum;
...
export class ReviewerController {
  ...
  async respond(@Param("assignmentId") assignmentId: string, @Body() body: unknown, @CurrentUser() user: CurrentUserType) {
  ...
  async submit(@Param("assignmentId") assignmentId: string, @Body() body: unknown, @CurrentUser() user: CurrentUserType) {
...
export class ReviewerCompatController {
  ...
  async respondCompat(@Param("assignmentId") assignmentId: string, @Body() body: unknown, @CurrentUser() user: CurrentUserType) {
    const dto = RespondDto.parse(body);
    return this.reviewer.respond(assignmentId, user.id, dto.response);
  }
  ...
  async submitCompat(@Param("assignmentId") assignmentId: string, @Body() body: unknown, @CurrentUser() user: CurrentUserType) {
    const dto = SubmitReviewDto.parse(body);
    return this.reviewer.submitReview(assignmentId, user.id, dto);
  }
}
